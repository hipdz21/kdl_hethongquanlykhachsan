...
public interface BillServiceRepository extends JpaRepository<BillServiceEntity, Integer> {
    ...
    @Query(value = "SELECT\n"
            ...
            + "AND YEAR(tbl_bill_service.bill_date) = ?2\n"
            + "GROUP BY tbl_service.service_name", nativeQuery = true)
    public List<Object[]> getNumberServiceInMonth(int month, int year);
    ...
    @Query(value = "SELECT\n"
            ...
            + "AND YEAR(tbl_bill_service.bill_date) = ?2\n"
            + "GROUP BY tbl_service.service_name", nativeQuery = true)
    public List<Object[]> getTotalRevenueServiceInMonth(int month, int year);
}
